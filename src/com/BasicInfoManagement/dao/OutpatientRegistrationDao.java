	...
	//保存修改收费种类信息
		public void saveRegistration(JC_SF_OutpatientRegistration jc_sf_outpaintientRegistration){
			...
			try {
				...
				session.saveOrUpdate(jc_sf_outpaintientRegistration);		//持久化Payment
...
//		
	//通过id查找门诊收费种类的信息	
		public JC_SF_OutpatientRegistration getRegistration(Integer id){
			Session session = null;					//Session对象
			JC_SF_OutpatientRegistration message = null;					//Message对象
			try {
				//获取Session
				session = HibernateUtil.getSession();
				session.beginTransaction();			//开启事物
		...
		public List<JC_SF_OutpatientRegistration> findRegistration(String type, String data){
			Session session = null;					//Session对象
			List<JC_SF_OutpatientRegistration> list = null;				//List集合
			try {
				//获取Session
				session = HibernateUtil.getSession();
				session.beginTransaction();			//开启事物
		...
		//查询所有门诊收费
			public List<JC_SF_OutpatientRegistration> findAllRegistration(){
					...
					try {
						...
						session = HibernateUtil.getSession();
						session.beginTransaction();			//开启事物
						String hql = "from JC_SF_OutpatientRegistration";		//HQL查询语句
						list = session.createQuery(hql)		//创建Query对象
							          .list();				//获取结果集
						session.getTransaction().commit(); 	//提交事物
					} catch (Exception e) {
			...
			//	System.out.println("11111111111111111111111111");
				...
				//	@SuppressWarnings("unchecked")
					List<JC_SF_OutpatientRegistration> list = session.createQuery(hql)		//创建Query对象
